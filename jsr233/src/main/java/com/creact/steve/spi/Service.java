...
/**
 * A simple service-provider lookup mechanism. A <i>service</i> is a well-known
 * set of interfaces and (usually abstract) classes. A <i>service provider</i>
 * is a specific implementation of a service. The classes in a provider
 * typically implement the interfaces and subclass the classes defined in the
 * service itself. Service providers may be installed in an implementation of
 * the Java platform in the form of extensions, that is, jar files placed into
 * any of the usual extension directories. Providers may also be made available
 * by adding them to the applet or application class path or by some other
 * platform-specific means.
 * 
 * <p>
 * In this lookup mechanism a service is represented by an interface or an
 * abstract class. (A concrete class may be used, but this is not recommended.)
 * A provider of a given service contains one or more concrete classes that
 * extend this <i>service class</i> with data and code specific to the provider.
 * This <i>provider class</i> will typically not be the entire provider itself
 * but rather a proxy that contains enough information to decide whether the
 ...
 * them, so no such class has been defined. The only requirement enforced here
 * is that provider classes must have a zero-argument constructor so that they
 * may be instantiated during lookup.
 * 
 * <p>
 * A service provider identifies itself by placing a provider-configuration file
 * in the resource directory <tt>ANDROID-META-INF/services</tt>. The file's name should
 * consist of the fully-qualified name of the abstract service class. The file
 * should contain a list of fully-qualified concrete provider-class names, one
 * per line. Space and tab characters surrounding each name, as well as blank
 * lines, are ignored. The comment character is <tt>'#'</tt> (<tt>0x23</tt>); on
 ...
 * <tt>ANDROID-META-INF/services/java.io.spi.CharCodec</tt>. This file would contain the
 * single line:
 * 
 * <pre>
 ...
 * caller. Trusted system code should typically invoke the methods in this class
 * from within a privileged security context.
 * 
...
public final class Service {
    ...
    private static void fail(Class service, String msg, Throwable cause)
        ...
        com.creact.steve.spi.ServiceConfigurationError sce = new com.creact.steve.spi.ServiceConfigurationError(
                service.getName() + ": " + msg);
        sce.initCause(cause);
        throw sce;
    }
    ...
    private static void fail(Class service, String msg)
            throws ServiceConfigurationError {
        throw new com.creact.steve.spi.ServiceConfigurationError(service.getName() + ": " + msg);
    }
    ...
    private static int parseLine(Class service, URL u, BufferedReader r,
            int lc, List names, Set returned) throws IOException,
     ...
     *
     * @param service
     *            The service class for which providers are being sought; used
     *            to construct error detail strings
     *
     * @param u
     *            The URL naming the configuration file to be parsed
     *
     * @param returned
     *            A Set containing the names of provider classes that have
            ...
            if (configs == null) {
                try {
                    ...
                    if (loader == null) {
                        ...
                        System.out.println("ClassLoader.getSystemResources: " + fullName);
                    } else {
                        configs = loader.getResources(fullName);
                        System.out.println("loader.getSystemResources: " + fullName + ", loader: " + loader);
                    }
                } catch (IOException x) {
                    fail(service, ": " + x);
                }
     ...
     *
     * @throws ServiceConfigurationError
     *             If a provider-configuration file violates the specified
     *             format or names a provider class that cannot be found and
     *             instantiated
     *
     * @see #providers(Class)
